...
impl MinimaxParameters {
    ...
    pub fn set_var_from_string(&mut self, identifier: String, val: String) {
        match &identifier[..] {
            ...
            "var_start" => {
                self.var_start = val.parse().expect("Got wrong val");
            }
            "var_end" =>  {
                self.var_end = val.parse().expect("Got wrong val");
            }
            "swarm_start" => {
            ...
            "futility_margin" => {
                self.futility_margin = val.parse().expect("Got wrong val");
            }
            _ => {
