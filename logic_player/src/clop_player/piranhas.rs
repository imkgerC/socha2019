...
impl Piranhas {
    ...
    pub fn get_winner(&self) -> Option<PlayerColor> {
        let own_size = self.state.greatest_swarm_size(&PlayerColor::Red);
        let own_len = self
            .state
            .board
            .get_fields_of(&PlayerColor::Red)
            .count_ones() as u8;
        let other_size = self.state.greatest_swarm_size(&PlayerColor::Blue);
        let other_len = self
            .state
            .board
